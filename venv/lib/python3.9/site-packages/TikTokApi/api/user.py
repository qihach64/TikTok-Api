...
class User:
    ...
    async def videos(self, count=10000, cursor=0, **kwargs) -> Iterator[Video]:
        """
        ...
        found = 0
        while True and found < count:
            params = {
                "secUid": self.sec_uid,
                "count": 30,
                "cursor": cursor,
            }
            print(f"🔍试图从该用户的视频列表里获取30个视频\n")
            ...
            print(f'🔍这次实际获取了{len(resp.get("itemList", []))}个视频 已经获取了{found}个视频 {"还有更多" if has_more else "没有了"}\n')
